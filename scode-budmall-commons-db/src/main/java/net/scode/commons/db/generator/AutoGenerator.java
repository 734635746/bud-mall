...
public class AutoGenerator {
    ...
    public void generate(GeneratorConfig config, String tableName, String entityName, boolean returnId) {
        ...
        generateDao(config, tableName, entityName, returnId);
        generateMapper(config, tableName, entityName, returnId);
        generateService(config, tableName, entityName, returnId);
    }
     ...
     * @param entityName
     */
    public void generateEntity(GeneratorConfig config, String tableName, String entityName) {
        JdbcUtil jdbc = null;
        try {
            ...
            if (list != null) {
                ...
                for (Map<String, Object> row : list) {
                    // 统一转驼峰
                    String field = StringUtil.toCamelCase((String) row.get("COLUMN_NAME"));
     ...
     * @param returnId
     */
    public void generateDao(GeneratorConfig config, String tableName, String entityName, boolean returnId) {
        try {
            ...
            if (returnId) {
                ibuf.append("    /**\n     * 插入一条记录, 并携带返回自增主键id的值\n     *\n");
                ibuf.append("     * @param entity\n     * @return\n     */\n");
                ibuf.append("     void insertReturnId(" + entityName + " entity);\n\n");
            }
            ibuf.append("}");
     ...
     * @param returnId
     */
    public void generateMapper(GeneratorConfig config, String tableName, String entityName, boolean returnId) {
        ...
        if (returnId) {
            String returnIdSql = doReturnIdSql(config, tableName, entityName);
            buf += returnIdSql;
        }
        buf += "</mapper>";
        FileUtil.writeUtf8String(buf, codeFullName);
     ...
     * @param returnId
     */
    public void generateService(GeneratorConfig config, String tableName, String entityName, boolean returnId) {
        try {
            ...
            if (returnId) {
                ibuf += "    /**\n     * 插入一条记录, 并携带返回自增主键id的值\n     *\n";
                ibuf += "     * @param entity\n     * @return\n     */\n";
                ibuf += "     void insertReturnId(" + entityName + " entity);\n\n";
            }
            ibuf += "}";
            FileUtil.writeUtf8String(ibuf.toString(), codeFullName);
            System.out.println(codeFullName + "生成完毕。");
            ...
            if (returnId) {
                ibuf += "    @Override\n";
                ibuf += "    public void insertReturnId(" + entityName + " entity) {\n";
                ibuf += "        this.getBaseMapper().insertReturnId(entity);\n";
                ibuf += "    }\n\n";
            }
            ibuf += "}";
